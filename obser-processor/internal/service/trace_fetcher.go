...
func (s *Service) ProcessTraces(ctx context.Context, traces [][]*types.SpanResponse) error {
...
func (s *Service) ProcessTrace(ctx context.Context, trace []*types.SpanResponse) error {
	root, _ := s.ConvertTraceToGraph(ctx, trace)
			...
			if span.ParentId != "" {
				...
				if parentSpan != nil {
					...
					_, err = tx.Run(ctx, query, map[string]interface{}{
						...
						"parentService":   parentSpan.LocalEndpoint.ServiceName,
						"childOperation":  span.Name,
						"childService":    span.LocalEndpoint.ServiceName,
						"pathId":          pathId,
					})
					if err != nil {
						return nil, err
...
func convertSrToSpan(sr *types.SpanResponse) *types.Span {
	...
	span.ServiceName = sr.LocalEndpoint.ServiceName
...
func (s *Service) FetchTracesFromTo(ctx context.Context, from int64, to int64, limit int) ([][]*types.SpanResponse, error) {
	...
	url = fmt.Sprintf("http://%s:%d/api/trace", config.TRACE_HOST, config.TRACE_PORT)
